...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('tools.indexed_pages_checker') }}" method="post" enctype="multipart/form-data" id="indexed-pages-checker-form" @cannot('researchTools', ['App\Models\User']) class="position-absolute left-5 right-5 opacity-20" @endcannot>
            ...
            <div class="row mx-n2">
                <div class="col px-2">
                    @if(config('settings.captcha_indexed_pages_checker'))
                        {!! NoCaptcha::displaySubmit('indexed-pages-checker-form', __('Search'), ['data-theme' => (config('settings.dark_mode') == 1 ? 'dark' : 'light'), 'data-size' => 'invisible', 'class' => 'btn ' . ($errors->has('g-recaptcha-response') ? 'btn-danger' : 'btn-primary')]) !!}
